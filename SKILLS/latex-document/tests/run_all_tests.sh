...
run_test_suite() {
  ...
  # Python suites require pytest; skip gracefully when unavailable.
  if [[ "$test_script" == *.py ]] && ! python3 -m pytest --version >/dev/null 2>&1; then
    echo -e "${YELLOW}[SKIP]${NC} $suite_name - pytest not installed"
    SUITE_NAMES+=("$suite_name")
    SUITE_RESULTS+=("SKIP")
    SUITE_DURATIONS+=("0.00")
    SKIPPED_SUITES=$((SKIPPED_SUITES + 1))
  ...
  case "$test_script" in
    *.py)
      if python3 -m pytest "$test_script"; then
        exit_code=0
      else
        exit_code=$?
      fi
...
print_summary() {
  ...
  # Calculate total time when bc is available
  if [[ "$HAS_BC" == true ]]; then
    local total_time=0
    ...
    echo -e "Total time:         ${YELLOW}n/a (bc not installed)${NC}"
  fi
